...
## Paper 10 - Restaurant Management System - Tư duy & Hướng dẫn chi tiết từng bước
...
│                                                          │
│  2. Xem ERD Diagram (Picture 1.1 và Database Schema)    │
...
|---------|------|-----------------|
| "retrieve", "display", "show", "list" | Lấy dữ liệu | SELECT |
| "which/where/who" | Điều kiện lọc | WHERE |
| "distinct", "duplicate records are not repeated" | Loại bỏ trùng | DISTINCT |
| "even if no customer reserved" | Bao gồm cả NULL | LEFT JOIN |
| "starts with" | Bắt đầu bằng | LIKE 'Nguyen%' |
| "include" | Bao gồm | LIKE '%keyword%' |
| "below the average" | Thấp hơn TB | AVG() + subquery/HAVING |
...
Hình bầu dục đôi      = Multi-valued attribute → Tạo bảng riêng
...
- Double line → Total participation (mọi entity phải tham gia relationship)
...
SELECT [cột_cần_lấy]
FROM [bảng]
WHERE (Role = 'X' OR Role = 'Y')  -- Dùng ngoặc khi kết hợp OR/AND
  AND [điều_kiện_khác];
```

**⚠️ Lưu ý:** Dùng ngoặc `()` khi kết hợp OR và AND!
...
## 📌 QUESTION 4: LEFT JOIN + LIKE FILTER [1 điểm]

### 📖 Đọc đề
> "List all tables whose locations include 'Khu A', 'Khu B' or 'Khu C', along with customers who reserved them in September 2025. Include tables even if no customer reserved them. Sort by Capacity DESC, then Location ASC"

### 🧠 Tư duy

```
Yêu cầu: Hiển thị bàn ở Khu A/B/C + khách đặt (tháng 9/2025)
         BAO GỒM cả bàn không có khách → LEFT JOIN
...
| Keyword | Action |
|---------|--------|
| "locations include 'Khu A', 'Khu B' or 'Khu C'" | Location LIKE '%Khu A%' OR ... |
| "even if no customer reserved" | LEFT JOIN |
| "September 2025" | MONTH() = 9 AND YEAR() = 2025 |
| "Sort by Capacity DESC, then Location ASC" | ORDER BY Capacity DESC, Location ASC |
...
LEFT JOIN Reservations r ON rt.ReservationID = r.ReservationID
    AND MONTH(r.ReservationTime) = 9
    AND YEAR(r.ReservationTime) = 2025
...
FROM Tables t
LEFT JOIN ReservationTables rt ON t.TableID = rt.TableID
LEFT JOIN Reservations r ON rt.ReservationID = r.ReservationTime
    AND MONTH(r.ReservationTime) = 9 AND YEAR(r.ReservationTime) = 2025
LEFT JOIN Customers c ON r.CustomerID = c.CustomerID
WHERE t.Location LIKE N'%Khu [ABC]%'
ORDER BY t.Capacity DESC, t.Location ASC;
```
...
```sql
LIKE '%Khu A%'     -- Chứa "Khu A" ở bất kỳ đâu
LIKE 'Khu A%'      -- Bắt đầu bằng "Khu A"
LIKE '%Khu A'      -- Kết thúc bằng "Khu A"
LIKE 'Khu [ABC]%'  -- Khu A, Khu B, hoặc Khu C (pattern)
```
...
```sql
-- ❌ SAI: Điều kiện trong WHERE sẽ biến LEFT JOIN thành INNER JOIN
LEFT JOIN Orders o ON c.CustomerID = o.CustomerID
WHERE YEAR(o.PaymentTime) = 2025  -- Sai!
...
## 📌 QUESTION 6: AGGREGATE + BELOW AVERAGE [1 điểm]

### 📖 Đọc đề
> "List all customers whose total spending on completed orders in 2025 is BELOW THE AVERAGE total spending of ALL customers (including those with no orders). Display CustomerID, FullName, TotalAmount. Sort by TotalAmount DESC, then CustomerID ASC"

### 🧠 Tư duy

```
Yêu cầu: Tìm khách có chi tiêu THẤP HƠN TRUNG BÌNH
         Trung bình tính trên TẤT CẢ khách (kể cả khách chi 0)
Input:   Customers, Orders
Output:  CustomerID, FullName, TotalAmount
```
...
| Bước | Xác định |
|------|----------|
| 1. Tính chi tiêu từng khách | LEFT JOIN + SUM(TotalAmount) |
| 2. Tính trung bình TẤT CẢ | AVG() trên kết quả bước 1 |
| 3. Lọc khách < TB | WHERE TotalAmount < (SELECT AVG(...)) |
...
SELECT CustomerID, FullName, TotalAmount
...
LEFT JOIN Orders o ON c.CustomerID = o.CustomerID
    ...
    AND YEAR(o.PaymentTime) = 2025
GROUP BY c.CustomerID, c.FullName
HAVING ISNULL(SUM(o.TotalAmount), 0) < (
    SELECT AVG(ISNULL(SUM(o2.TotalAmount), 0))
    FROM Customers c2
    LEFT JOIN Orders o2 ON c2.CustomerID = o2.CustomerID
        AND o2.Status = 'Completed'
        AND YEAR(o2.PaymentTime) = 2025
    GROUP BY c2.CustomerID
)
ORDER BY TotalAmount DESC, c.CustomerID ASC;
```
...
Hoặc viết trong 1 block:

```sql
-- Cách 1: Tách 2 câu INSERT
...
-- Character class
LIKE 'Khu [ABC]%'     -- Khu A, Khu B, hoặc Khu C
```
...
"include 'Khu A/B/C'"         → LIKE '%Khu A%' OR ...
"distinct, not repeated"      → DISTINCT
"even if not ordered/reserved" → LEFT JOIN
"average, below average"       → AVG() + subquery/HAVING
...
### Q4: Cách dùng LIKE với nhiều giá trị?

```sql
-- Cách 1: Nhiều OR
WHERE col LIKE '%A%' OR col LIKE '%B%' OR col LIKE '%C%'

-- Cách 2: Character class (chỉ 1 ký tự)
WHERE col LIKE '%Khu [ABC]%'

-- Cách 3: IN (không dùng được với LIKE)
WHERE col IN ('A', 'B', 'C')  -- Không dùng % được
```
...
| Sai thứ tự OR/AND | `WHERE A OR B AND C` | `WHERE (A OR B) AND C` |
...
| LIKE sai pattern | `LIKE 'Khu A'` | `LIKE '%Khu A%'` |
...
### Bài tập 2: LIKE filter
> "Hiển thị bàn ở 'Khu A' hoặc 'Khu B'"

```sql
SELECT * FROM Tables
WHERE Location LIKE N'%Khu A%' OR Location LIKE N'%Khu B%';
```
...
### Bài tập 4: Below Average
> "Tìm khách có chi tiêu dưới trung bình"

```sql
WITH Spending AS (
    SELECT c.CustomerID, SUM(o.TotalAmount) AS Total
    FROM Customers c
    LEFT JOIN Orders o ON c.CustomerID = o.CustomerID
    GROUP BY c.CustomerID
)
